...
@Component({
  selector: 'app-configure-categories-section',
...
export class ConfigureCategoriesSectionComponent implements OnInit, OnDestroy {
  ...
  saveEdit(category: CategoryDto): void {
    ...
    this.categoriesService.updateCategory(category.id, { name, color: this.editColor }).pipe(takeUntil(this.destroy$)).subscribe({
      next: (updated) => {
        this.categories = this.categories.map((existing) => existing.id === category.id ? updated : existing);
        this.cancelEdit();
  ...
  deleteCategory(category: CategoryDto): void {
    ...
    this.categoriesService.deleteCategory(category.id).pipe(takeUntil(this.destroy$)).subscribe({
      next: () => {
        this.categories = this.categories.filter((existing) => existing.id !== category.id);
